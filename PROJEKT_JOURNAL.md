...
## v0.8.4 — Changelog-Seite (automatisch aus diesem Journal generiert)
- Vierter Schritt des Untermenü-Ausbaus, vorgezogen vor History/Design, um gleich als Testfall für den
  neuen GitHub-Update-Mechanismus (v0.8.3) zu dienen: eine Version, die über die WordPress-Oberfläche selbst
  aktualisiert wird, statt nur per `wp-cli` lokal.
- **Umgesetzt:** `ftipp_parse_changelog_md()` liest `fussball-tippspiel/CHANGELOG.md` zeilenweise, erkennt
  Versions-Überschriften (`## v<Version> — <Titel>`), überspringt nicht-versionierte Zwischenüberschriften
  und bricht am `## OFFENE AUFGABEN`-Abschnitt ab. Da Versionsnummern in diesem Journal nicht eindeutig/
  monoton sind (mehrfache v0.5.0/0.6.0/0.7.0, dazwischen ein v1.0.0-Ausflug), sortiert die Anzeige
  ausschließlich nach Position in der Datei (umgedreht = neueste zuerst), nie nach Versionsvergleich.
  `ftipp_changelog_md_to_html()` ist ein bewusst einfacher, in sich geschlossener Markdown-Konverter
  (escaped zuerst alles über `esc_html()`, ersetzt danach gezielt `` `code` ``, `**fett**`, `*kursiv*` und
  `[Text](Link)`, gruppiert `- `-Zeilen zu Listen) — es gibt im Plugin bisher keinen wiederverwendbaren
  Markdown-Renderer, daher neu geschrieben statt eine weitere Bibliothek einzubinden.
- **Neuer Release-Schritt ab sofort:** Da `PROJEKT_JOURNAL.md` nur im Projektordner liegt, nicht im
  Plugin-Zip, wird vor jedem `zip -r` zusätzlich `cp PROJEKT_JOURNAL.md fussball-tippspiel/CHANGELOG.md`
  ausgeführt — der Parser filtert Einleitung und TODO-Liste ohnehin automatisch heraus, eine gekürzte
  Fassung ist nicht nötig (Zielgruppe ist ausschließlich der Plattform-Admin, technischer Ton ist in Ordnung).
- Lokal getestet: alle 30+ Versionseinträge werden korrekt geparst und in umgekehrter Dateireihenfolge
  angezeigt, Markdown-Formatierung (fett/kursiv/Code/Links/Listen) rendert sauber, keine rohen HTML-Escapes
  sichtbar.
- **Live-Test des kompletten Update-Zyklus:** dieses Release wird bewusst über die normale
  WordPress-Oberfläche ("Update verfügbar" → "Jetzt aktualisieren" im Plugins-Bereich) eingespielt statt
